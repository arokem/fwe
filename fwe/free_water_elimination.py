...
layout.get(subject=subject, session=session, scope="synthetic")
...
logger.debug(f"Found bval file at: {bval_fp[0].path}")
bvals, bvecs = read_bvals_bvecs(str(bval_fp[0].path), str(bvec_fp[0].path))
...
if len(unique_bvals) == 0:
    ...
    img_fp = layout.get(**context, suffix="dwi", extension="nii.gz")[0].path
    img = nib.load(img_fp)
    ...
    # Load the brain mask
    brain_mask_fp = layout.get(**context, suffix="mask", extension="nii.gz")[0].path
    if not brain_mask_fp:
        logger.warning("Brain mask not found for the specified subject and session.")
    else:
        logger.debug(f"Found brain mask at: {brain_mask_fp}")
        brain_mask = nib.load(brain_mask_fp).get_fdata()
    # load tissue segmentation mask
    tissue_seg_fp = layout.get(**context, suffix="dseg", extension="nii.gz")[0].path
    ...
    logger.info("This is multi-shell data.")
