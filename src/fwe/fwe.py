...
def free_water_elimination(
    ...
    match fwe_model.lower():
        ...
        case "beltrami":
            logger.info(
                "Performing free water elimination using Golub's Beltrami model"
            )
            fwe_image, model_params = single_shell_beltrami(
                dwi, gtab, mask, Diso, save_params, **opt_kwargs
            )
        case _:
...
def single_shell_beltrami(
    ...
    model_params = (
        nib.Nifti1Image(model_fit.model_params, affine=dwi.affine)
        if save_params
        else None
    )
...
def dipy_fwdti(dwi, gtab, mask=None, Diso=3.0e-3, save_params=False):
